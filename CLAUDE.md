...
When breaking down a plan file into bd tasks, each task must contain enough context for an agent to implement it **without reading the entire plan**. Follow these rules:

- **Always add a `--notes` field** with a reference to the plan file, specific section name, and line range (e.g., `"Full spec: docs/path/to/plan.md — Section 'Step 3' (lines 120-180)"`)
- **Cross-reference related Research sections** from the plan if the task needs background context (e.g., `"Also read Research section: 'Financial Integration' (lines 59-75)"`)
- **Note shared artifacts** — if a task creates classes/services used by later tasks, mention this (e.g., `"Creates ReceivedPaymentCreator used by Steps 5 and 6"`)
- **Description should list key deliverables**: files to create/modify, method signatures to implement, classes to register — but the plan file remains the source of truth for full logic details
